...
AVATAR_PATH = os.path.join(os.path.dirname(__file__), "..", "fixtures", "avatar.png")
TMP_DIR = tempfile.gettempdir()

# token authentication from config file
gl = gitlab.Gitlab.from_config(config_files=[f"{TMP_DIR}/python-gitlab.cfg"])
gl.auth()
...
import_archive = f"{TMP_DIR}/gitlab-group-export.tgz"
...
with open(f"{TMP_DIR}/gitlab-export.tgz", "wb") as f:
    ex.download(streamed=True, action=f.write)

output = gl.projects.import_project(
    open(f"{TMP_DIR}/gitlab-export.tgz", "rb"),
    "imported_project",
